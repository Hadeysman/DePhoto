...
if(!class_exists('DePhoto')){
	class DePhoto {
		...
		function login($redirect) {
			...
			if ( !empty ( $_POST ) ) {
				...
				if ( $subpass == $stopass ) {
					...
					setcookie('dephotologauth[user]', $subname, 0, '', '', '', true);
					setcookie('dephotologauth[authID]', $authID, 0, '', '', '', true);
		...
		function logout() {
			...
			$idout = setcookie('dephotologauth[authID]', '', -3600, '', '', '', true);
			$userout = setcookie('dephotologauth[user]', '', -3600, '', '', '', true);
